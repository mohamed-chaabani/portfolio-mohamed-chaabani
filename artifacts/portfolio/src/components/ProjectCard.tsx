...
export function ProjectCard({ title, description, tech, githubUrl, liveUrl, delay = 0 }: ProjectCardProps) {
      ...
      <div>
        <div className="flex justify-between items-start mb-6">
          ...
          <div className="flex gap-3 text-muted-foreground">
            {githubUrl && (
              <a href={githubUrl} target="_blank" rel="noopener noreferrer" className="hover:text-primary transition-colors p-2 hover:bg-primary/10 rounded-full" aria-label="GitHub Repository">
                <Github size={20} />
              </a>
            )}
            {liveUrl && (
              <a href={liveUrl} target="_blank" rel="noopener noreferrer" className="hover:text-primary transition-colors p-2 hover:bg-primary/10 rounded-full" aria-label="Live Demo">
